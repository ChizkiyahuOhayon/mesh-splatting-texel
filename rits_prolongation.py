...
def donor_mode(donor_window, donor_opacity):
    """Encode the two mechanism groups as the rasterizer's donor bitmask."""
    return (DONOR_WINDOW if donor_window else 0) | (DONOR_OPACITY if donor_opacity else 0)


def install_prolongation_probe(model, selected_faces, donor_window, donor_opacity):
    ...
    mode = donor_mode(donor_window, donor_opacity)
    probe["window_donors"] = (

#!/usr/bin/env bash
# SoftTail v3 three-scene A/B: softmin routing of the shared opacity gradient.
#
# The pooled arm is the frozen v1 configuration plus --opacity_pool, so the
...
RUNS=${RUNS:-$NAS_ROOT/experiments/softtail_v3_01}
GPU=${GPU:-0}
...
mkdir -p "$RUNS"
[ -f "$RUNS/DONE" ] && { echo "SoftTail v3 gate already complete: $RUNS"; exit 0; }

# 1. Training: one configuration for every scene.
for SCENE in "${SCENES[@]}"; do
  "$HERE/run.sh" pool "$SCENE" --final_opacity 0.8 --opacity_pool
done

# The pooled checkpoint must be structurally a v1 checkpoint: the contribution is
# a gradient change, so any new field here would mean the forward model moved.
for SCENE in "${SCENES[@]}"; do
...
assert state.get("opacity_floor_vertex") is None, "pooled arm must not carry a per-vertex floor"
print(sys.argv[1], "floor:", float(state["opacity_floor"]), "vertices:", state["vertex_weight"].shape[0])
  ' "$RUNS/pool__${SCENE}/point_cloud/iteration_30000/point_cloud_state_dict.pt"
done
...
for SCENE in "${SCENES[@]}"; do
  for ARM in ours_quality ours_speed ours_opacity; do
    ...
    "$MESH_SPLATTING_PYTHON" -u -m sota.main_table_eval \
      -s "$DATA_ROOT/$SCENE" -m "$RUNS/pool__${SCENE}" -i "$(images_for "$SCENE")" --eval \
      --scene "$SCENE" --arm "$ARM" --iteration 30000 --output "$OUTPUT"
  done
done

"$MESH_SPLATTING_PYTHON" -m sota.v3_gate \
  "$FORMAL_ROOT/formal_table.json" "$RUNS" "${SCENES[@]}"

touch "$RUNS/DONE"
echo "SoftTail v3 three-scene A/B complete: $RUNS/gate.json"

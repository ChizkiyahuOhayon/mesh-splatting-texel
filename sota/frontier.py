"""Quality against model size for every finished arm.
    ...
    python -m sota.frontier [scene] [runs_dir]
...
RUNS = sys.argv[2] if len(sys.argv) > 2 else "/root/autodl-tmp/runs"

rows = []
for run in sorted(glob.glob(f"{RUNS}/*__{SCENE}")):
...
if base is None:
    sys.exit(f"no base run for {SCENE} under {RUNS}")
print(f"scene: {SCENE}")

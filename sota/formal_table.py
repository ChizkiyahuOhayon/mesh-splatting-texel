"""Aggregate the formal nine-scene stock and method evaluations."""
...
PAPER = {"psnr": 24.78, "ssim": 0.728, "lpips_vgg": 0.310}
...
def main(root):
    ...
    deltas = {
        arm: {
            key: means[arm][key] - means["stock"][key]
            for key in METRICS
        }
        for arm in ARMS[1:]
    }
    ...
    table = {
        "experiment": "formal-nine-scene-main-table-v1",
        "scenes": list(SCENES),
        "rows": rows,
        "means": means,
        "delta_mean_vs_stock": deltas,
        "win_counts_vs_stock": wins,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("usage: python -m sota.formal_table OUTPUT_ROOT")
    main(sys.argv[1])

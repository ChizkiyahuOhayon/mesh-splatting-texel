...
- **Cost, read from the run logs** (nine Mip-NeRF360 scenes, one A40, machines not isolated): training 1.95 h → 2.29 h mean (+17.3%) for the extra accumulator and the pre-cleanup save; render FPS 18.02 → 15.36 on the quality arm and 21.81 → 18.64 on the speed arm, which tracks the 2.6–33% larger meshes rather than the statistic itself.
